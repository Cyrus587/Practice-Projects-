...
public class RhymingDict {
	...
	// TO DO 2: Store a rhymeGroup (key) and word (value) in the Dictionary
	// (hashtable)
	public static void storeRhyme(DictionaryInterface rhymingDict, String line) {
		// make rhyme group and word variable
		String rhymeGroup = getRhymeGroup(line);
		String word = getWord(line);
		// lookup bucket in rhyming dict hash table
		MySortedLinkedList lookup = (MySortedLinkedList) rhymingDict.get(rhymeGroup);
		// if bucket is null
		if (lookup == null) {
			// create new bucket and add entry to bucket
			MySortedLinkedList newBucket = new MySortedLinkedList();
			newBucket.add(word);
			rhymingDict.put(rhymeGroup, newBucket);
		} else {
	...
	// Select two rhymeGroups, select two random words from each rhyme group, and
	// make a rhyme.
	public static void createRhyme(DictionaryInterface rhymingDict) {
		...
		if (random.nextFloat() > 0.5f)
			System.out.printf("\nRoses are %s,\nviolets are %s.\nI am %s\nand you are %s.\n", word0A, word1A, word0B,
					word1B);
		else
			System.out.printf("\nIf I were %s\nthen you'd be the %s,\nAnd we'd both be %s\nand never be %s\n", word0A,
					word1A, word0B, word1B);
	...
	public static void removeUnrhymables(DictionaryInterface rhymingDict) {
		...
		for(int i = 0; i < keys.length; i++) {
			...
			MySortedLinkedList currentList = (MySortedLinkedList) rhymingDict.get(currentKey); 
			if (currentList.size() ==1) {
				rhymingDict.remove(currentKey);
			}
		}
	}

	// Once you've implemented your dictionary, you can use the testDictionary
	// function to test
	...
	public static void main(String[] args) {
		...
		// Save a seed.
		// Like in Lab01, the seeds allow us generate the same random numbers (the same
		// poems) again
		...
		/*
		 * String[] keys = rhymingDict.getKeys(); System.out.println(keys.length);
		 * System.out.println("Biggest bucket = " + rhymingDict.biggestBucket() +
		 * " average bucket = " + rhymingDict.averageBucket());
		 */
